	...
	<div class="container">
		...
		<table class="table table-hover">
			<thead
				...
				<tr>
					<th colspan="2" class="text text-center">Products</th>
					<th>Price</th>
			...
			%>
				...
				<tr>
					<td>
						<img src="./ShowImage?pid=<%=product.getProdId()%>" style="width: 50px; height: 50px;"></td>
					<td>
						<%=product.getProdName()%>
					</td>
					<td>
						<%=String.format("%,d", product.getProdPrice()).replace(",", ".") %>
					</td>
					<td>
					...
					<td>
						<a href="cartDetails.jsp?add=1&uid=<%=userName%>&pid=<%=product.getProdId()%>&avail=<%=product.getProdQuantity()%>&qty=<%=prodQuantity%>"><i class="fa fa-plus"></i></a>
					</td>
					<td>
						<a href="cartDetails.jsp?add=0&uid=<%=userName%>&pid=<%=product.getProdId()%>&avail=<%=product.getProdQuantity()%>&qty=<%=prodQuantity%>"><i class="fa fa-minus"></i></a>
					</td>
					<td>
						<%= String.format("%,.0f", currAmount).replace(",", ".") %>
					</td>
				</tr>
			...
			%>
				...
				<tr style="background-color: grey; color: white;">
					<td colspan="6" style="text-align: center;">Total Amount to Pay (in VND)</td>
					<td><%= String.format("%,.0f", totAmount).replace(",", ".") %></td>
				</tr>
			<%
				if (totAmount != 0) {
			%>
				<tr style="background-color: grey; color: white;">
					...
					<td colspan="2" align="center">
						<form method="post">
							<button style="background-color: green; color: white;" formaction="payment.jsp?amount=<%=totAmount%>">Pay Now</button>
						</form>
					</td>
