...
public class OrderServiceImpl implements OrderService {
	...
	@Override
	public String paymentSuccess(String userName, int paidAmount) {
		...
		for (CartBean item : cartItems) {
			...
			int amount = new ProductServiceImpl().getProductPrice(item.getProdId()) * item.getQuantity();
	...
	public boolean addTransaction(TransactionBean transaction) {
		...
		try {
			...
			ps.setInt(4, transaction.getTransAmount());
		...
		try {
			...
			while (rs.next()) {
				...
				OrderBean order = new OrderBean(rs.getString("orderid"), rs.getString("prodid"), rs.getInt("quantity"),
						rs.getInt("amount"), rs.getInt("shipped"));
		...
		try {
			...
			while (rs.next()) {
				...
				OrderBean order = new OrderBean(rs.getString("t.transid"), rs.getString("t.prodid"),
						rs.getInt("quantity"), rs.getInt("t.amount"), rs.getInt("shipped"));
